...
namespace GroupProject
{
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            String dateString = thisDay.ToString("MM-dd-yyyy", System.Globalization.CultureInfo.InvariantCulture);
        ...
        private void Button6_Click(object sender, EventArgs e)
        {
            if (add)
            {
                ...
                {
                    Console.WriteLine(" start time" + startTimes[loop] + " end time" + endTimes[loop]);
            ...
            else
            {
                ...
                List<List<String>> times = new List<List<string>>();
                List<String> allEvents = new List<string>();
                selectedEvent.updateEventValue(textBox1.Text, textBox2.Text, textBox3.Text,
                   textBox4.Text, comboBox3.Text, thisDate, curUser.username, textBox7.Text);
                if (selectedEvent.getLocation() != "LOC-NA")
                {
                    allEvents.Add(selectedEvent.getLocation());
                }
                for (int i = 0; i < checkedListBox1.CheckedItems.Count; i++)
                {
                    //Console.WriteLine(checkedListBox1.CheckedItems[i].ToString());
                ...
                allEvents.Add(curUser.username);
                times = selectedEvent.GetTimes(allEvents, thisDate);
                //waiting a moment since the database did not always want to cooperate with my internet
                //TimeUnit.SECONDS.sleep(3);
                startTimes = times[0];
                endTimes = times[1];
                ...
                for (int loop = 0; loop < startTimes.Count; loop++)
                {
                    Console.WriteLine("Start Time: " + getValue(startTimes[loop]) + " End Time: " + getValue(endTimes[loop]));
                    ranges.Add(getRange(getValue(startTimes[loop]), getValue(endTimes[loop])));
                }
                if (endTimes[endTimes.Count - 1] != "24:00")
                {
        ...
        {
            String thisDate = monthCalendar1.SelectionRange.Start.ToString("MM-dd-yyyy");
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (selectedEvent.getOwner() != curUser.username) //no the owner of event = no editing event
            {
                ...
                message = "You did not select an event.";
                caption = "Edit Failed";
                buttons = MessageBoxButtons.OK;
                DialogResult result;
                // Displays the MessageBox.
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            if (res == 1)
            {
                ...
                String dateString = thisDay.ToString("MM-dd-yyyy", System.Globalization.CultureInfo.InvariantCulture);
                label1.Text = "Events on " + dateString;
                dateString = thisDay.ToString("yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            string message = "The new event edited in the database successfully.";
            ...
            try
            {
                ...
                Console.WriteLine("Connecting to MySQL...");
                conn.Open();
                string sql = " UPDATE gbmevent SET title = @title, starttime = @st, endtime = @et, reminder = @rem, description =@desc WHERE eventid =@cevent; ";
                MySql.Data.MySqlClient.MySqlCommand cmd = new MySql.Data.MySqlClient.MySqlCommand(sql, conn);
                ...
                cmd.Parameters.AddWithValue("@st", eventStartTime);
                cmd.Parameters.AddWithValue("@et", eventEndTime);
                cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
